...
class FullTextDataset(Dataset):
    ...
    def split_sents(self, text):
        ...
        while idx < len(sents):
            ...
            while running_length <= (self.max_len - 2) and sent_idxs:
                ...
                selected_idx.append(idx)
                idx += 1
            selected_sents.append(selected_idx)
        
        selected_texts = []
        for sent_idxs in selected_sents:
            selected_text = ''
    ...
    def __getitem__(self, index):
        ori_text = str(self.text[index])
        ori_text = " ".join(ori_text.split())

        id, mask, token_type_ids = [], [], []
        splitted_texts = self.split_sents(ori_text)
