...
from transformers import LongformerTokenizer, BertTokenizer, RobertaTokenizer
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu", type=int, default=3, help="Random seed")
    parser.add_argument("--hidden_size", type=int, default=768, help="Hidden embedding size of LaficMIL")
    ...
    elif args.model_name.lower() == 'laficmil-posemb':
        model = models.LaficMIL(num_labels, device=torch.device("cuda:{}".format(device[0])), pos_type='PosEmb', hidden_size=args.hidden_size)
        dataset_class = datasets.FullTextDataset

    elif args.model_name.lower() == 'laficmil-no-pos':
        model = models.LaficMIL(num_labels, device=torch.device("cuda:{}".format(device[0])), pos_type=None, hidden_size=args.hidden_size)
        dataset_class = datasets.FullTextDataset
    ...
    logging.basicConfig(filename='basic_logs/'+output_model_name + '.log', level=logging.DEBUG)
